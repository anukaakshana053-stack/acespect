// One-off content overlay: publishes a v2 for the Pre-Purchase + Apartment
// profile's job-info, roof_chimneys, elevations, garage_carport_sheds,
// internal_areas and notes sections, ported from the Houspect "Building
// (Pre-Purchase) Inspector Template_APARTMENT" (7 April 2025). This content
// is near-identical to the Dilapidation + Apartment overlay built earlier
// (same building-condition checklist items apply regardless of inspection
// purpose) -- the pieces that differ are the job-info intake fields
// specific to a pre-purchase inspection (occupied, agent/client on site,
// renovations, client list of issues). Every other (inspectionType,
// propertyType) lineage is untouched; uses the existing versioning flow.
...
const INTERNAL_SYSTEM_FIELDS: Omit<TemplateField, 'order'>[] = [
  ...
  ...subArea({
    prefix: 'intstairs', group: 'Internal Stairs to Unit',
...
async function publishOverlay(sectionKey: string, buildFields: (existing: TemplateField[]) => TemplateField[]) {
  ...
  console.log(`[prepurchase-apartment] published ${sectionKey} v${draft.version} (${nextFields.length} fields)`);
}
